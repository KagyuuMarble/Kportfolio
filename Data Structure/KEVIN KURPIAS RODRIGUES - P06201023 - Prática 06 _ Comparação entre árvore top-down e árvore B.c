...
// Encontra o nó no qual uma determinada chave está ou deveria estar
...
// Função auxiliar para inserir um valor em uma árvore B - Números
//repetidos não são inseridos
struct no *insere(struct no *atual, int info, bool &h, int
&info_retorno) {
int i, pos,
...
if (atual == NULL) {
//O no anterior e o ideal para inserir a nova chave (chegou emum no folha)
h = true;
info_retorno = info;
return NULL;
} else {
...
//Números repetidos não são inseridos
...
insere_chave_B(temp, atual->dado[pos_mediano],
atual->filhos[pos_mediano+1]);
}
else {
...
insere_chave_B(temp, atual->dado[i],
atual->filhos[i+1]);
//atualiza na atual.
